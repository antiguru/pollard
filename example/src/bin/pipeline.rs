//! Three-stage event pipeline with three independent defects.
//!
//! Each stage has its own slow and fast implementation; slow mode runs all
//! three slow paths, fast mode runs all three fast paths. The checksum
//! emitted at the end is identical in both modes — the work each pair does
//! is observationally equivalent, only the cost differs.
//!
//! Stage A — `route`: build a per-event routing key from `(host, metric)`.
//!   Slow: `format!("{}:{}", host, metric)` allocates a `String` per event,
//!         then we byte-sum it.
//!   Fast: byte-sum derived directly from the integers (no allocation).
//!
//! Stage B — `digest`: per-window p99 extraction from a value vector.
//!   Slow: full `sort_unstable` to read one element.
//!   Fast: `select_nth_unstable` in place.
//!
//! Stage C — `rollup`: sum a `[GRID_ROWS][GRID_COLS]` u32 grid.
//!   Slow: outer col, inner row — strides `GRID_COLS * 4` bytes between
//!         reads, misses cache on every step.
//!   Fast: outer row, inner col — unit-stride, prefetcher-friendly.
//!
//! Three hot functions, three different fix patterns (allocator hygiene,
//! algorithm, memory layout). Use this when the demo needs a profile with
//! more than one peak — leaf-frame attribution alone won't surface all
//! three at once because the costs are spread across distinct call paths.
//!
//! Run:  `cargo run --profile demo --bin pipeline -- slow`
//!       `cargo run --profile demo --bin pipeline -- fast`
