...
- *(query)* reject malformed pid:/tid: prefix instead of name fallback
- *(query)* reject unknown sort_by/group_by/align_by values
- *(query)* wire `time_range` filter through sample iteration
- *(query)* wire process filter (name + sub-pid)

### Other

- enable clippy::match_wildcard_for_single_variants
- drop redundant `#![allow(dead_code)]`
