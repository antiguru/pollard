...
impl PollardServer {
    ...
    pub async fn load_profile(
        ...
        let id = self.registry.load(&args.path, args.name.as_deref()).await?;
        ...
        Ok(Json(LoadProfileResult { profile_id: id, description: desc }))
    }
    ...
    pub async fn describe_profile(
        ...
        let session = self
            ...
            .get(&args.profile_id)
            .await
            .ok_or_else(|| ToolError::ProfileNotFound { profile_id: args.profile_id.clone() })?;
        Ok(Json(describe(
            session.profile(),
            session.id(),
            session.name(),
